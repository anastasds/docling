...
DoclingDocument, returns a stream of chunks, each of which
...
- def chunk(self, dl\_doc: DoclingDocument, **kwargs) -> Iterator[BaseChunk]:
...
the DoclingDocument to create one chunk for each individual
detected document element, by default only merging together list items (can be opted out
via param merge\_list\_items). It also takes care of attaching all relevant document
